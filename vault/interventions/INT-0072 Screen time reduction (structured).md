---
id: 'INT-0072'
type: 'intervention'
name: 'Screen time reduction (structured)'
category: 'lifestyle'
modality: 'lifestyle'
directionality: 'prevention'
csrs_score: 43.09
csrs_prevention_score: 56.47
csrs_treatment_score: 45.93
status: 'active'
otc_or_rx: 'lifestyle'
pediatric_safe: 'yes'
cost_per_month_usd: 0
dose_range: '<1h/day age 2-5; none under 2 (AAP)'
last_updated: '2026-06-24T18:46:42+00:00'
---

# INT-0072 Screen time reduction (structured)


Limiting screen exposure in 0-3y per AAP guidelines; preserve social interaction windows.


**Dose range:** <1h/day age 2-5; none under 2 (AAP)


**Cost per month (USD):** 0


**Notes / warnings:** Free; low risk.


## Hypotheses

- [[HYP-0038 Excessive screen smartphone exposure (early life)]]

## Mechanisms

- [[MEC-0006 Synaptic pruning abnormalities]]

## Phenotypes

- [[PHE-0005 mTOR pathway syndromic (TSC, PTEN)]]
- [[PHE-0006 Fragile X (FMR1)]]
