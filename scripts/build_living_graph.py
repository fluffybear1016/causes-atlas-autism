      ...
      try {
        ...
        if (matched.length === 0) {
          ...
          gStatus.textContent = res.format + ' · ' +
            res.variants.length.toLocaleString() + ' variants read · 0 matched to atlas genes. ' +
            'This file may use a chip that excludes the rsIDs the atlas tracks — try uploading a PDF report from the same provider for richer gene-symbol detection.';
          return;
        }
        gStatus.textContent = res.format + ' · ' +
          res.variants.length.toLocaleString() + ' variants read · ' +
          matched.length + ' matched to atlas genes · generating your report…';
        saveGenetics({
          format: res.format,
          uploadedAt: Date.now(),
